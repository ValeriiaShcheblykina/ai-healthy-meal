...
async function globalTeardown() {
  ...
  if (result.error) {
    ...
    console.info('✓ Loaded .env.test file');
  }
  ...
  if (!hasSupabaseUrl || !hasServiceKey) {
    console.error(
      '⚠️  Skipping cleanup - required environment variables are missing'
    );
    console.error(
      'Make sure .env.test exists in the project root and contains:'
    );
