...
import { ApiError, createErrorResponse, createUnauthorizedError } from '@/lib/errors/api-errors'
...
export const GET: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const response: RecipeListResponseDTO = await recipeService.listRecipes(validationResult.data)
    ...
    return new Response(
      JSON.stringify(createErrorResponse('INTERNAL_ERROR', 'An unexpected error occurred')),
      {
        status: 500,
        headers: {
          'Content-Type': 'application/json',
