...
export const GenerateRecipeFromPreferences = memo(
  function GenerateRecipeFromPreferences({
    disabled = false,
    variant = 'secondary',
    size = 'default',
    className = '',
    ...
    useEffect(() => {
      const checkPreferences = async () => {
        try {
          ...
          if (profile) {
            // Memoize preference checks
            const hasDiets = Boolean(profile.diets?.length);
            const hasAllergens = Boolean(profile.allergens?.length);
            const hasDislikedIngredients = Boolean(
              profile.dislikedIngredients?.length
            );
    ...
    const buttonText = useMemo(
      () =>
        isGenerating ? 'Generating...' : 'Generate Recipe from My Preferences',
      [isGenerating]
    );
    const buttonTextMobile = useMemo(
